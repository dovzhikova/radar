...
import { routeOutcomeRank, coverageBannerTone, inClusterOutcome, inClusterEligible, RequestIndicator } from './TracePanel'
import { ReachabilityExplainer } from './ReachabilityExplainer'
import { completedRequestMode } from './ReachabilityView'
...
describe('protocol-honest probe copy', () => {
  ...
  it('labels TCP routes as TCP in the test matrix', () => {
    ...
    const html = renderToStaticMarkup(createElement(ReachabilityExplainer, { trace, probed: true }))
    expect(html).toContain('No connection ran')
    expect(html.match(/>TCP</g)).toHaveLength(2)
    expect(html).not.toContain('GET /')
